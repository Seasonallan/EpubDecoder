...
import com.season.bookreader.ui.ReaderMediaPlayer.Frame;
	...
	public void drawContent(Canvas canvas,int left, int top, int right, int bottom){
		...
		if(state == ReaderMediaPlayer.STATE_START){
			if(mPlayDrawable == null){
				mPlayDrawable = MyAndroidApplication.getInstance().getResources().getDrawable(R.drawable.ic_reader_voice_play);
			}
			...
			if(state != ReaderMediaPlayer.STATE_PREPAREING){
				if(mPauseDrawable == null){
					mPauseDrawable = MyAndroidApplication.getInstance().getResources().getDrawable(R.drawable.ic_reader_voice_pause);
				}
